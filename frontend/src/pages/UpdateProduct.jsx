...
import { backendBase, backendUrl } from "../utils/commonItems";
...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await axios(`${backendUrl}get_single_product/${ProductId}`);
        setRequestObj({
        ...
        setSelectedFiles([`${backendBase}${res.data.data.file_path}`])
      } catch (error) {
        setvisible(true);
        toastChangeFunct(
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const res = await axios(`${backendUrl}update_product/${ProductId}?_method=PUT`, {
        method: "POST",
        data: formData,
      });
        ...
        <div className="w-fit mt-[20px] flex flex-row items-center justify-between">
          <div className="mt-2  w-[90px] h-[90px] border-2 border-dashed border-white ">
            <img
              src={
                val?
                URL.createObjectURL(selectedFiles[0]):selectedFiles[0]
              }
